		...
		foreach ($filters as $filter)
		{
			...
			switch ($filter['type'])
			{
				...
				case 'category':
                    ...
                    AND c.nright < '.(int)$parent->nright.') OR (c.nleft > 43 AND c.nright < 72))
                    '.($depth ? 'AND c.level_depth <= '.($parent->level_depth+(int)$depth) : '').'
                    AND c.active = 1
                    GROUP BY c.id_category ORDER BY c.nleft, c.position';
                     */
                    if ((int)$parent->nright < 43) {
                        // art category
                        $sql_query['group'] .= 'WHERE c.nleft > 43
					    AND c.nright < 72
					    AND c.active = 1
					    GROUP BY c.id_category ORDER BY c.nleft, c.position';
                    } else if ((int)$parent->nleft > 43) {
                        // usage space
                        $sql_query['group'] .= 'WHERE c.nleft > 21
					    AND c.nright < 42
					    AND c.active = 1
					    GROUP BY c.id_category ORDER BY c.nleft, c.position';
                    }
...
//					GROUP BY c.id_category ORDER BY c.nleft, c.position';
			...
			{
				$sql_query['from'] .= Shop::addSqlAssociation('product', 'p');//p($sql_query['select']."\n".$sql_query['from']."\n".$sql_query['join']."\n".$sql_query['where']."\n".$sql_query['group']);
					...
					if (isset($products) && $products)
					{
						...
						if ($categories_with_products_count || !Configuration::get('PS_LAYERED_HIDE_0_VALUES')) {
                            ...
                            $filter_blocks[] = array (
								'type_lite' => 'category',
								'type' => 'category',
								'id_key' => 0, 'name' => $name, //$this->l('Categories'),
								'values' => $tmp_array,
								'filter_show_limit' => $filter['filter_show_limit'],
								'filter_type' => $filter['filter_type']
							);
